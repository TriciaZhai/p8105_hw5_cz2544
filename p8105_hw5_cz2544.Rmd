...
city = murder %>% 
  ...
  replace_na(list('Closed by arrest' = 0, 'Closed without arrest' = 0, 'Open/No arrest' = 0)) %>% 
...
city_nest = city %>% 
  replace_na(list(n_unsolve = 0, n_total = 0)) %>% 
 # filter(n_unsolve*n_total != 0) %>% 
  nest(n_unsolve, n_total) %>% 
  mutate( p = map(data, prop_t),
          outcome = map(p, broom::tidy))%>% 
  select(city_state, outcome) %>%
  unnest() %>% 
